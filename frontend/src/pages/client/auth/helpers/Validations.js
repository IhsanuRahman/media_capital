...
function SignupValidator(errors, userData) {
    ...
    if (userData.last_name.trim() === '') {
        ...
        if (errors.last_name.length > 15) {
            errors.last_name = 'only maximum 15 chacters is allowed in first name'
        } else
            errors.last_name = ''
    }
    if (userData.email.trim() === '') {
        ...
        if (!regex.test(userData.email.trim())) {
            ...
            is_valid = false
        }else if (userData.email.length > 32) {
            errors.email = 'only maximum 32 chacters is allowed in first name'
            is_valid=false
        } else {
            errors.email = ''
        }
    ...
    if (userData.password.trim()!==userData.conform_password.trim()){
